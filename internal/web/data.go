...
// handleDataOverview returns every pool with its squares, grouped into the fixed
// static (instance) pools and the guild's shared pools.
func (s *Server) handleDataOverview(w http.ResponseWriter, r *http.Request) {
	gid := r.PathValue("gid")
	if _, ok := s.requireDataAdmin(w, r, gid); !ok {
		return
	}
	static, err := s.poolsWithEntries(r, gid, store.KindInstance)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "could not load data")
		return
	}
	shared, err := s.poolsWithEntries(r, gid, store.KindShared)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "could not load data")
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"static": static, "shared": shared})
}

func (s *Server) poolsWithEntries(r *http.Request, guildID, kind string) ([]map[string]any, error) {
	pools, err := s.store.ListPools(r.Context(), guildID, kind)
	if err != nil {
		return nil, err
	}
...
func (s *Server) handleDataPoolCreate(w http.ResponseWriter, r *http.Request) {
	...
	pool, err := s.svc.CreateSharedPool(r.Context(), gid, userID, body.Slug, body.Name)
	if err != nil {
		writeServiceError(w, err)
		return
	}
...
// handleDataPoolDelete removes a shared pool and its squares.
func (s *Server) handleDataPoolDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.svc.DeleteSharedPool(r.Context(), gid, userID, body.PoolID); err != nil {
		writeServiceError(w, err)
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"ok": true})
